    ...
    def __init__(
        ...
        self.network = keras.Sequential(
            [
                ...
                layers.Conv2D(
                    kernel_size=2, filters=4, padding="same", strides=(1, 1), activation="relu",
                ),
                tf.keras.layers.BatchNormalization(),
                layers.Conv2D(
                    kernel_size=2, filters=8, padding="same", strides=(1, 1), activation="relu",
                ),
                tf.keras.layers.BatchNormalization(),
                layers.Conv2D(
                    kernel_size=2, filters=16, padding="same", strides=(1, 1), activation="relu",
                ),
                tf.keras.layers.BatchNormalization(),
                layers.Conv2D(
                    kernel_size=2, filters=8, padding="same", strides=(1, 1), activation="relu",
    ...
    for num_epochs in num_epochs_list:
        ...
        for i in tqdm.tqdm(range(num_epochs)):
            ...
            if i % 10 == 0:
                ...
                print()
        player_0.save("player_large_{}_3/".format(num_epochs))
